...
def load_predefined_character(character_name: str) -> None:
    ...
    # - Upstream uses "Normal"
    # - easytts may localize it to "普通"
    default_key = "Normal" if "Normal" in prompt_wav_dict else ("普通" if "普通" in prompt_wav_dict else None)
    if default_key is None:
        # Fallback to the first entry.
        try:
            default_key = next(iter(prompt_wav_dict.keys()))
        except StopIteration:
            raise ValueError("prompt_wav.json is empty")

    audio_text = prompt_wav_dict[default_key]["text"]
    audio_path = os.path.join(save_path, "prompt_wav", prompt_wav_dict[default_key]["wav"])
    _reference_audios[character_name] = {
        'audio_path': audio_path,
        'audio_text': audio_text,
        'language': CHARA_LANG[character_name],
